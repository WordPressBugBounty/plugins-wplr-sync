...
if ( !class_exists( 'MeowKit_WPLR_Admin' ) ) {

  class MeowKit_WPLR_Admin {
    ...
    public function __construct( $prefix, $mainfile, $domain, $isPro = false, $disableReview = false, $freeOnly = false ) {
      ...
      if ( is_admin() ) {
        ...
        if ( MeowKit_WPLR_Helpers::is_asynchronous_request() ) {
          return;
        }
    ...
    public static function process_deferred_instances() {
      ...
      if ( !function_exists( 'current_user_can' ) || !function_exists( 'wp_get_current_user' ) ) {
        trigger_error(
          'MeowKit_WPLR_Admin: Pluggable functions still unavailable on init hook. ' .
    ...
    private function run_admin_setup() {
      ...
      if ( !MeowKit_WPLR_Admin::$loaded ) {
        // Check for potential issues with WordPress install, other plugins, etc.
        new MeowKit_WPLR_Issues( $this->prefix, $this->mainfile, $this->domain );
        ...
        MeowKit_WPLR_Admin::$loaded = true;
      }
      ...
      if ( $this->is_user_admin() ) {
        if ( !$this->disableReview ) {
          new MeowKit_WPLR_Ratings( $this->prefix, $this->mainfile, $this->domain );
        }
        new MeowKit_WPLR_News( $this->domain );
      }
    }
    ...
    public function admin_menu_start() {
      ...
      if ( !isset( $submenu[ 'meowapps-main-menu' ] ) ) {
        add_menu_page(
          'Meow Apps',
          '<img alt="Meow Apps" style="width: 21px; margin-left: -28px; position: absolute; margin-top: 2px;" src="' . MeowKit_WPLR_Admin::$logo . '" />Meow Apps',
          'manage_options',
          'meowapps-main-menu',
    ...
    public function meowapps_hide_dashboard_callback() {
      ...
      echo MeowKit_WPLR_Helpers::wp_kses( $html );
    }
